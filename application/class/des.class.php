...
***************************************************/
/**
type of mode (0 for ECB and 1 for CBC, ECB is the default), an optional 8 character string input vector (not used in ECB mode),
type of padding (0 for null or zero bytes, 1 for PKCS7, 2 for spaces and 3 for no padding at all). 
*/
class des
{
	var $key = '';
	...
	var $padding = false;
	function des(){
		$this->setMode('ECB', 'PKCS5');
	}
	function setKey($key){
		$this->key = $key;
	}
	
	function setMode($mode, $padding){
		$m = array('ECB'=>0,'CBC'=>1);
		//0 for null or zero bytes, 1 for PKCS7, 2 for spaces and 3 PKCS5).		
		$p = array('NULL'=>0,'PKCS7'=>1,'SPACE'=>2,'PKCS5'=>3);
		$this->mode		= isset($m[$mode])		? $m[$mode]		: 0 ;
		$this->padding	= isset($p[$padding])	? $p[$padding]	: 0 ;
	}
	...
	function encrypt($data,$key=NULL){
		$k = $key===NULL ? $this->key : $key;
		$ciphertext = $this->_des ($k, $data, 1, $this->mode, null , $this->padding );
	...
	function decrypt($data,$key=NULL){
		$k = $key===NULL ? $this->key : $key;
		//$msg = $this->hexToString($data);
		$msg = base64_decode($data);
		return $this->_des ($k, $msg, 0, $this->mode, null , $this->padding);
	}
	
	//this takes the key, the message, and whether to encrypt or decrypt
	function _des ($key, $message, $encrypt, $mode, $iv, $padding) {
	  ...
	  if ($padding == 3) {$message = $this->pkcs5_pad($message);} //PKCS5 padding
	  else if ($padding == 2) $message .= "        "; //pad the message with spaces
	  ...
	  return $this->unpad($result . $tempresult, $padding);
	} //end of des
