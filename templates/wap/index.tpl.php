...
<?php TPL::plugin('wap/include/my_preview', '', false); ?>
	...
	<div class="send">
		<div><span><?php LO('index__default__newsTellSomeBody');?></span></div>
	...
	<div class="row">
	<?php LO('index__default__all');?> <a href="<?php echo WAP_URL('index.favorites'); ?>"><?php LO('index__default__favs');?></a>
	</div>
	<?php if (!empty($list)): ?>
	<?php TPL::plugin('wap/include/feedlist', array('list' => $list), false); ?>
	<?php else: ?>
		<?php if (V('g:page', 1) > 1):?>
		<p><?php LO('index__default__endPage');?></p>
		<?php else: ?>
