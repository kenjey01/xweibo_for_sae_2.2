...
<body class="main-body">
    ...
    <script type="text/javascript">
    	function setWbState()
    	{
        	...
        	jQuery.post('<?php echo URL("mgr/micro_interview.setWbState");?>', '<?php echo "id=$interviewId&state="; ?>'+state);
    	}
        
        
        var counter = $('.approve-tips .num');
    	function getWbCnt(){
        	jQuery.getJSON('<?php echo URL("mgr/micro_interview.countWb", array('id'=>$interviewId) );?>', function(data){
        		if( data.errno == 0 )
